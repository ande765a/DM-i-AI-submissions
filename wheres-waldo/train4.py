...
from models import TransferModel2
...
model = TransferModel2().to(device) #BaselineCNN(in_channels=3).to(device)
...
try:
  for epoch in range(num_epochs):
    ...
    for image, _, (x, y) in test_data:
      ...
      px = (preds % image.shape[3]).item() * 4
      py = (preds / image.shape[2]).item() * 4
